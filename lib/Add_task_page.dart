...
class Add_task extends StatefulWidget {
  ...
  const Add_task({super.key, this.loc});

  @override
  State<Add_task> createState() => _Add_task_Page();
}

class _Add_task_Page extends State<Add_task> {
  ...
  Directions() {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        // Search_Address("출발지", _startlocation), // 도로명 주소 (출발지)
        // Search_Address("도착지", _endlocation), // 도로명 주소 (도착지)
        (km != "" && time != "")
            ? Container(
  ...
  Search_Address(String point, TextEditingController point_Address) {
    // 출발지, 목적지 주소 검색
    return Container(
      height: 30,
      margin: EdgeInsets.only(top: 10),
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: TextField(
              controller: point_Address,
              style: TextStyle(fontSize: 13, color: Colors.blue, height: 2),
              decoration: InputDecoration(
                hintStyle: TextStyle(fontSize: 12, height: 1.3),
                ...
                suffixIcon: GestureDetector(
                  child: Icon(Icons.location_on_sharp, color: Colors.green, size: 18),
                  onTap: () async {
                    Kpostal result = await Navigator.push(context, MaterialPageRoute(builder: (_) => KpostalView(kakaoKey: 'yourKey')));
                    if (point == "출발지") {
                      setState(() {
      ...
      child: Container(
        ...
        child: ElevatedButton(
          ...
          onPressed: _isTitle_Text && (_isToday || _isTime)
              ? () {
                  _addTaskToDb(); // 받은 일정을 데이터베이스에 넣는 과정
